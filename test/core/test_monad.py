...
class MyMaybe(Monad):
    ...
    def __eq__(self, other):
        return (
            isinstance(other, MyMaybe)
            and self.is_nothing == other.is_nothing
...
def test_monad_impl_function():
    ...
    assert (
        Applicative.apply(f, lambda x: add(x))(42)
        == (lambda x: add(x)(x + 1))(42)
        == "4243"
    )
    assert (
        Monad.bind(f, lambda x: add(x))(42) == (lambda x: add(x + 1)(x))(42) == "4342"
    )
...
def test_monad_impl_function_ext():
    ...
    assert Functor.map(map, list)(lambda x: x + 1, [1, 2, 3, 4]) == list(
